...
function omitFromSchema(schema, ...properties) {
    ...
    const newSchema = lodash_omit_1.default(JSON.parse(JSON.stringify(schema)), properties);
...
class Spec {
    ...
    addModels(models) {
        for (const [name, schema] of Object.entries(models)) {
            this.models[(schema.ref || name).replace(/^models[/.]/, '')] = lodash_omit_1.default(schema, 'ref');
        }
    }
    ...
    addFolder(folder) {
        for (const file of glob_1.sync(`${folder}/**/*(*.ts|*.js)`)) {
            ...
            for (const route of routes) {
                if (route) {
                    const models = lodash_get_1.default(route, 'config.models');
                    const securitySchemes = lodash_get_1.default(route, 'config.securitySchemes');
                    if (models) {
