...
function serializedErrorDescription(error) {
    return `[${error.code || error.name}] ${error.message}`;
}
exports.serializedErrorDescription = serializedErrorDescription;
...
function toBoomError(error, data) {
    ...
    return boom_1.internal('', { message: serializedErrorDescription(error), stack });
}
