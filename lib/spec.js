...
function omitFromSchema(schema, ...properties) {
    ...
    const newSchema = utils_1.omit(JSON.parse(JSON.stringify(schema)), properties);
...
class Spec {
    constructor({ title, description, authorName, authorUrl, authorEmail, license, version, servers, tags, folder }, skipDefaultErrors = false) {
        ...
        this.errors = Object.values(skipDefaultErrors ? {} : errors_1.errors).reduce((accu, e) => {
            accu[e.properties.statusCode.enum[0]] = utils_1.omit(e, 'ref');
            return accu;
        }, {});
        if (folder) {
    ...
    addModels(models) {
        for (const [name, schema] of Object.entries(models)) {
            this.models[(schema.ref || name).replace(/^models[/.]/, '')] = utils_1.omit(schema, 'ref');
        }
    }
    ...
    addRoutes(routes) {
        ...
        const apiRoutes = routes
            .filter((r) => {
            const schema = utils_1.get(r, 'schema', {});
            const config = utils_1.get(r, 'config', {});
            return !schema.hide && !config.hide;
        })
            .sort((a, b) => a.url.localeCompare(b.url));
        // For each route
        for (const route of apiRoutes) {
            const schema = utils_1.get(route, 'schema', {});
            const config = utils_1.get(route, 'config', {});
    ...
    addFolder(folder) {
        for (const file of glob_1.sync(`${folder}/**/*(*.ts|*.js)`)) {
            ...
            for (const route of routes) {
                if (route) {
                    const models = utils_1.get(route, 'config.models');
                    const securitySchemes = utils_1.get(route, 'config.securitySchemes');
                    if (models) {
    ...
    parseParameters(schema) {
        ...
        for (const [section, where] of Object.entries(exports.parametersSections)) {
            ...
            const required = utils_1.get(specs, 'required', []);
            // For each property
            for (const [name, spec] of Object.entries(utils_1.get(specs, 'properties', {}))) {
                params.push({
                    name,
                    in: where,
    ...
    resolveReference(schema, ...keysBlacklist) {
        ...
        return utils_1.omit(schema, ['ref', '$ref'].concat(keysBlacklist));
    }
    generateSchemaObjects(object, prefix) {
        return Object.entries(object).reduce((accu, [k, v]) => {
            accu[`${prefix}.${k}`] = utils_1.omit(v, ['ref', '$ref']);
            return accu;
        }, {});
    }
}
